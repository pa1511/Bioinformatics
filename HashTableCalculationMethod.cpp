...
int HashTableCalculationMethod::POW_4_VALUE[] = {
      1, 4, 16, 64, 256, 1024, 4096, 16384, 65536, 262144, 1048576, 4194304,
      16777216, 67108864, 268435456, 1073741824
};

int HashTableCalculationMethod::INV_HASH_MASK[] = {
      0x0, 0x3, 0xf, 0x3f, 0xff, 0x3ff, 0xfff, 0x3fff, 0xffff, 0x3ffff, 0xfffff,
      0x3fffff, 0xffffff, 0x3ffffff, 0xfffffff, 0x3fffffff
};
...
HashTable* HashTableCalculationMethod::calculate(Document* document, int w,
          int k, int threadCount) {
  ...
  if (threadCount == 1) {
    ...
    for (int i = 0; i < threadCount; i++) {
      std::thread* worker = new std::thread(
              &HashTableCalculationMethod::calculateTask,
...
void HashTableCalculationMethod::calculateTask(SyncQueue<BioSequence*>& tasks,
  ...
  while (true) {
    ...
    minimizerSketch(sequence, w, k,
            *taskResult.minimizerSet0, *taskResult.minimizerSet1);
...
int HashTableCalculationMethod::PHI_function(std::string *seqence,
...
void HashTableCalculationMethod::minimizerSketch(bioinformatics::BioSequence *sequence,
        int w, int k, std::vector<Minimizer>& M0, std::vector<Minimizer>& M1) {
