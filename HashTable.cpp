        ...
        for (auto it = keys.begin(); it != keys.end(); it++) {
            ...
            if (h0It != this->hashTableRaw0->end()) {
                for (auto setit = h0It->second->begin(); setit != h0It->second->end(); setit++) {
                    hashFile << ((int)setit->sequencePosition) << " "
                             << setit->i << " " << 0 << std::endl;
                }
            }
    ...
    if (hashFile.is_open()) {
        while (getline(hashFile, line)) {
            ...
            if (line.at(0) == '#') {
                ...
                if (r == 0) {
                    std::unordered_map<int, std::vector<bioinformatics::Entry>*>::iterator mapIt =
                            hashTable0->find(std::stoi(key));
                    ...
                    if (mapIt != hashTable0->end()) {
                        ...
                        hashTable0->insert(
                            std::pair<int, std::vector<bioinformatics::Entry>*>
                                (
                                    std::stoi(key),
                                    entrySet
                                )
